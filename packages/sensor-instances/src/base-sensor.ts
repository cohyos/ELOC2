...
// ---------------------------------------------------------------------------
// SensorInstance — Abstract Base Class
// ---------------------------------------------------------------------------

/**
 * Abstract base class for all sensor models in the ELOC2 distributed
 * sensor architecture. Each SensorInstance owns its own local TrackManager
 * and RegistrationHealthService, receives ground-truth broadcasts via the
 * SensorBus, and publishes track/bearing reports back upstream.
 *
 * Concrete subclasses (RadarSensor, EoSensor, C4isrSensor) implement
 * the abstract `tick()` and `filterTargetByCoverage()` methods.
 */
export abstract class SensorInstance {
  ...
  protected positionHistory: Map<
    string,
    Array<{ lat: number; lon: number; alt: number; timeSec: number }>
  > = new Map();
  ...
  protected buildStatusReport(): SensorStatusReport {
    const health = this.registrationService.getHealth(
      this.sensorId as SensorId,
    );
    return {
      sensorId: this.sensorId as SensorId,
      sensorType: this.sensorType,
      online: this.online,
      mode: this.mode,
      trackCount: this.localTrackManager
        .getAllTracks()
  ...
  protected recordPositionHistory(
    trackId: string,
    pos: Position3D,
    timeSec: number,
  ): void {
    ...
    if (history.length > SensorInstance.MAX_POSITION_HISTORY * 2) {
      this.positionHistory.set(
        trackId,
        history.slice(-SensorInstance.MAX_POSITION_HISTORY),
      );
    }
  }
